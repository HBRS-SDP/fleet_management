...
from fleet_management.plugins.mrta.timetable_monitor import (
    TimetableMonitor,
    TimetableMonitorProxy,
)
from fleet_management.plugins.mrta.schedule_execution_monitor import (
...
class FMSBuilder:
    ...
    def configure(self, config):
        for c in self.config_order:
            ...
            component = self.configure_component(
                c, **component_config, **self._components
            )
...
class RobotBuilder:
    ...
    def robot_store(self, robot_id, robot_store_config):
        ...
        robot_store_config["db_name"] = (
            robot_store_config["db_name"] + "_" + robot_id.split("_")[1]
        )
        robot_store = Store(**robot_store_config)
        return robot_store

    def __call__(self, robot_id, allocation_method, config):
        self._factory = MRTABuilder(
            allocation_method, component_modules=self._component_modules
        )
        self._factory.register_component("api", self.api(robot_id, config.pop("api")))
        self._factory.register_component(
            "robot_store", self.robot_store(robot_id, config.pop("robot_store"))
        )
...
robot_proxy_builder.register_component_module(
...
robot_builder.register_component_module(
...
class PluginBuilder:
    ...
    def configure(self, key, **kwargs):
        ...
        if key == "mrta":
            ...
            builder = builder(
                self.allocation_method, component_modules=component_modules
            )
...
plugin_factory.register_builder("topology", topology.configure)
