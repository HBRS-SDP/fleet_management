...
class TaskPlannerInterface(object):
    ...
    def __init__(self, kb_database_name, domain_file, planner_cmd, plan_file_path, **_):
        ...
        self.planner_interface = LAMAInterface(
            kb_database_name=kb_database_name,
    ...
    def plan(self, request, path_planner):
        ...
        """
        formatted_dict = Message.from_model(request, meta_model_prefix=None).get(
            "payload"
        )
        formatted_dict["pickupLocationLevel"] = self._get_location_floor(
            formatted_dict.get("pickupLocation")
        )
        formatted_dict["deliveryLocationLevel"] = self._get_location_floor(
            formatted_dict.get("deliveryLocation")
        )
    ...
    def _get_task_plan_without_robot(self, task_request: TaskRequest, path_planner):
        ...
        self.kb_interface.insert_fluents([robot_location_fluent, cart_location_fluent])
        ...
        try:
            ...
            if task_request.load_type == "mobidik":
                task_goals = [
                    (
                        "load_at",
                        [("load", load_id), ("loc", task_request.delivery_pose.name)],
                    ),
            ...
            plan_found, actions_ = self.planner_interface.plan(
                task_request, robot_name, task_goals
            )
            if plan_found:
    ...
    def _plan_paths(self, task_plan: list, path_planner):
        ...
        for i in range(len(task_plan)):
            ...
            if action.type.lower() == "exit_elevator":
                ...
                previous_sub_area = path_planner.get_sub_area(
                    action.areas[0].name,
...
def initialize_knowledge_base(kb_database_name):
    ...
    elevator_facts = [
        ("elevator_at", [("elevator", "elevator0"), ("loc", "BRSU_A_L0_A8")]),
        ("elevator_at", [("elevator", "elevator0"), ("loc", "BRSU_A_L2_A1")]),
    ]
    kb_interface.insert_facts(elevator_facts)

    elevator_fluents = [
        ("elevator_floor", [("elevator", "elevator0")], "floor100"),
        ("destination_floor", [("elevator", "elevator0")], "floor100"),
    ]
    kb_interface.insert_fluents(elevator_fluents)

    elevator_location_fluents = [
        ("location_floor", [("loc", "BRSU_A_L0_A8")], "floor0"),
